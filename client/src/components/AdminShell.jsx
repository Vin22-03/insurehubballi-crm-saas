...
function AdminShell({ title, subtitle, activeTab = "dashboard", children }) {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.12),_transparent_28%),linear-gradient(180deg,#edf6ff_0%,#eef7ff_45%,#f7fbff_100%)] text-slate-900">
      <div className="mx-auto w-full max-w-[1700px] px-4 py-4 sm:px-5 sm:py-5 lg:px-6">
        <div className="flex min-h-[calc(100vh-2rem)] flex-col overflow-hidden rounded-[30px] border border-blue-100/80 bg-white/60 shadow-[0_25px_70px_rgba(30,64,175,0.08)] backdrop-blur-xl sm:min-h-[calc(100vh-2.5rem)]">
          <header className="border-b border-blue-100/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.78),rgba(240,247,255,0.94))]">
            <div className="px-5 py-5 sm:px-6 lg:px-8">
              ...
              <div className="mt-5 hidden flex-wrap items-center gap-3 lg:flex">
                    ...
                    <button
                      ...
                      className={`rounded-2xl px-4 py-2.5 text-sm font-semibold transition ${
                        isActive
                          ...
                          : "border border-blue-100 bg-white/80 text-slate-700 hover:bg-blue-50"
                      }`}
                    >
                      {item.label}
                    </button>
                ...
                <div className="ml-auto flex items-center gap-3">
                  <div className="hidden rounded-2xl border border-blue-100 bg-white/80 px-4 py-2 text-sm text-slate-600 xl:block">
                    {user?.name || "Admin"}
                  </div>
                  ...
                  <button
                    ...
                    className="rounded-2xl border border-red-200 bg-white/80 px-4 py-2.5 text-sm font-semibold text-red-600 transition hover:bg-red-50"
                  >
                    Logout
                  </button>
                </div>
              ...
              {mobileOpen && (
                <div className="mt-4 rounded-3xl border border-blue-100 bg-white/90 p-4 shadow-sm lg:hidden">
          ...
          <footer className="border-t border-blue-100/80 bg-white/45 px-5 py-4 sm:px-6 lg:px-8">
            <div className="flex flex-col gap-2 text-center text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between sm:text-left">
              ...
              <p>Admin control center · Premium CRM</p>
            </div>
          </footer>
        </div>
      </div>
